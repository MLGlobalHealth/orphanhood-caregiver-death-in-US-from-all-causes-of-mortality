...
do.main <- as.data.table(read.csv(file.path(args$prj.dir, 'results',paste0('CI_', race.type, 'V0207'), 'initial_result', paste0('1-hist_national_race_fert_stable_summary_all_cg_loss_age.csv'))))
...
row.title <- paste0("Rate of cumulative burden of\nparental death per 100 children")
  ...
  pd[, type := ifelse(grepl('Main',  type), 'Central analysis', 'Sensitivity analysis on standardized race & ethnicity-specific\nnational-level fertility rates before 1990')]
...
tmp <- tmp[, list(year,cause.name,race.eth,child.age,grandp.loss,variable)]

# check:
tmp2 <- tmp[, list(grandp.loss.t = sum(grandp.loss, na.rm = T)),
...
if (1)
{
  ...
  # plot the incidence of grandp loss
  ...
  # tmp2 <- merge(tmp2, c.pop, by = c('year', 'age.group'), all.x = T)
  # tmp2[, num := value]
  #
  # tmp2[, value := value/pop*1e5]
  # tmp2[, value := value/10/100]
  ...
  pm
  ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('edf_sens_age2_distrib_children_losing_grandparents.png')), pm, w = 26, h = 13, dpi = 310, limitsize = FALSE)
  ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('edf_sens_age2_distrib_children_losing_grandparents.pdf')), pm, w = 26, h = 13, dpi = 310, limitsize = FALSE)
