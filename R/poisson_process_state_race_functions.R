...
process_adj_mort_data_cdc_state_race_poisson <- function(mort.data.raw, mort.data, cause.type, in.dir, prj.dir, imp.num)
{
  # imp.num <- 1
  ...
  # ))
  {
    ...
    get_adjusted_mort_data_state_level(prj.dir, in.dir, rep.nb, sample.type, imp.num)
...
process_adj_mort_data_cdc_state_race_unintentional_poisson <- function(mort.data.raw, mort.data, in.dir, prj.dir)
{
  cause.type <- 'unintentional_injuries'
  imp.num <- 3
  d.drug <- process_mort_data_cdc_state_race_unintentional(mort.data, in.dir, prj.dir, impute.supp = T, imp.num)
...
process_top5states_pry_cause_by_race_poisson <- function(mort.data.raw, mort.data, in.dir, prj.dir, imp.num)
{
  # process for each primary cause
  cat('Drug overdose ... \n')
  process_adj_mort_data_cdc_state_race_poisson(mort.data.raw, mort.data, cause.type = 'drug', in.dir, prj.dir, imp.num)
  cat('Diseases of heart ... \n')
  process_adj_mort_data_cdc_state_race_poisson(mort.data.raw, mort.data, cause.type = 'diseases_of_heart', in.dir, prj.dir, imp.num)
  cat('Unitentional injuries ... \n')
  process_adj_mort_data_cdc_state_race_poisson(mort.data.raw, mort.data, cause.type = 'unintentional_injuries', in.dir, prj.dir, imp.num)
...
# used ----
process_female_fertility_state_race_year_poisson = function(in.dir, type.input, impute.supp, pop, imp.num, rep.nb)
{
  if (!file.exists(
  ...
  if (rep.nb != 1)
  {
    ...
    set.seed(rep.nb)
    data_fertility[, births.rep := rpois(nrow(data_fertility), lambda = data_fertility$births)]
    data_fertility[, births := births.rep]
    set(data_fertility, NULL, 'births.rep', NULL)
  }
...
# used ----
process_male_fertility_state_race_year_poisson = function(in.dir, type.input, impute.supp, pop, imp.num, rep.nb)
{
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'usa_fertility_m_raw.rds'))
...
process_nchs_state_race_fertility_poisson <- function(in.dir, pop,rep.nb)
{
  ...
  # resample the birth data
  if (rep.nb != 1)
  {
    cat('Resample births data\n')

    set.seed(rep.nb)
    data.all.nchs[, births.rep := rpois(nrow(data.all.nchs), lambda = data.all.nchs$births)]
    data.all.nchs[, births := births.rep]
    set(data.all.nchs, NULL, 'births.rep', NULL)
  }
...
# fertility rates ----
# used ----
process_usa_states_race_birth_comp_poisson <- function(prj.dir, in.dir, type.input, imp.num, rep.nb)
{
  # type.input <- 'state_race'
  if( !file.exists(
  ...
  {
    process_female_fertility_state_race_year_poisson(in.dir, type.input, F, pop[sex == 'Female'],imp.num = imp.num, rep.nb)
    process_male_fertility_state_race_year_poisson(in.dir, type.input, F, pop[sex == 'Male'], imp.num = imp.num, rep.nb)
  }
  ...
  {
    process_female_fertility_state_race_year_poisson(in.dir, type.input, impute.supp = T, pop[sex == 'Female'],imp.num = imp.num, rep.nb)
    process_male_fertility_state_race_year_poisson(in.dir, type.input, impute.supp = T, pop[sex == 'Male'],imp.num = imp.num, rep.nb)
  }
  ...
  pop <- pop[age.cat != '0-14', list(population = sum(as.numeric(population), na.rm = T)),
             by = c('state','year','age.cat','sex','race.eth')]
  process_nchs_state_race_fertility_poisson(in.dir, pop, rep.nb)
...
# used ----
process_usa_states_race_birth_fertility_all_year_imputation_poisson <- function(prj.dir, in.dir, rep.nb, type.input)
{
  ...
  {
    set.seed(rep.nb)
    process_usa_states_birth_fertility_all_year_imputation_poisson(in.dir, 'state', rep.nb)
  }
  ...
  {
    set.seed(rep.nb)
    process_usa_states_race_birth_comp_poisson(prj.dir, in.dir, type.input, imp.num = 1, rep.nb)
  }
