...
if (tmp["user"] == "yc2819" & grepl("hpc.ic.ac.uk",tmp["nodename"])) # outdir yu
{
  option_list <- list(
    ...
    optparse::make_option("--pkg_dir", type = "character", default = NA_character_,
                          ...
                          dest = "prj.dir"),
    optparse::make_option("--nb_rep", type = "integer", default = 1e4,
                          help = "Absolute file path to package directory, used as long we don t build an R package [default]",
                          dest = "nb.rep")
  )
...
# Bootstrap for co-residenting grandparents ----
if (0)
{
  for (rep.nb in (1:args$nb.rep))
  {
    set.seed(rep.nb)
  ...
  get_95_ci_grandp_state <- function(in.dir, nb.rep)
  {
    all.df.race <- list()
    for (rep.nb in 1:nb.rep)
    {
  ...
  get_95_ci_grandp_national_race <- function(in.dir, nb.rep)
  {
    all.df.race <- list()
    for (rep.nb in 1:nb.rep)
    {
  ...
  get_95_ci_grandp_state( args$in.dir, args$nb.rep)
  get_95_ci_grandp_national_race( args$in.dir, args$nb.rep)
}
# 2407 second review separating gp loss into primary and secondary ----
# Bootstrap for primary-cg grandparents with the sampled proportion of providing care

for (rep.nb in (1:args$nb.rep))
{
  set.seed(rep.nb)
  # national race.eth level also by state
  get_grandp_household_cg_national_state_ACS_ci_sep(args$in.dir, rep.nb)
  get_grandp_household_cg_national_race_ACS_ci_sep(args$in.dir, rep.nb)
  cat('Finished the ', rep.nb, ' sampling for grandp data who are primary cg and providing essential care + secondary cg...\n')
}

get_95_ci_grandp_state_pry_cg <- function(in.dir, nb.rep)
{
  all.df.race <- list()
  for (rep.nb in 1:nb.rep)
  {
    cat('The ', rep.nb, '-th sample...\n')
    rep.dir <- paste0('rep_grandp-', rep.nb)
    data.dir <- file.path(in.dir, 'grandparents', rep.dir, paste0('skip_generation_state_national_total_pc_summary.csv'))
  ...
  data.all <- data.table::rbindlist( all.df.race, use.names = T, fill = T)
  set(data.all, NULL, 'sg', NULL)
  saveRDS(data.all, file.path(in.dir, 'grandparents', paste0('grandp_ACS_state_pc_sample_all.RDS')))

  rm(all.df.race)

  # compute for the 95% bootstrap intervals at STATE level
  # pds.quantiles <- c(.05,.5,.95)
  pds.quantiles <- c(.025,.5,.975)
  ...
  cat('\nSaving the grandp at state level with quantiles...\n')
  saveRDS(data.all.state, file.path(in.dir, 'grandparents', paste0('grandp_ACS_state_pc_ci.RDS')))
}

# custodial
get_95_ci_grandp_state_custodial_cg <- function(in.dir, nb.rep)
{
  all.df.race <- list()
  for (rep.nb in 1:nb.rep)
  {
    cat('The ', rep.nb, '-th sample...\n')
    rep.dir <- paste0('rep_grandp-', rep.nb)
    data.dir <- file.path(in.dir, 'grandparents', rep.dir, paste0('skip_generation_state_national_custodial_summary.csv'))
  ...
  data.all <- data.table::rbindlist( all.df.race, use.names = T, fill = T)
  set(data.all, NULL, 'sg', NULL)
  saveRDS(data.all, file.path(in.dir, 'grandparents', paste0('grandp_ACS_state_custodial_sample_all.RDS')))
  ...
  cat('\nSaving the grandp at state level with quantiles...\n')
  saveRDS(data.all.state, file.path(in.dir, 'grandparents', paste0('grandp_ACS_state_custodial_ci.RDS')))
}

# race level
get_95_ci_grandp_national_race_pry_cg <- function(in.dir, nb.rep)
{
  all.df.race <- list()
  for (rep.nb in 1:nb.rep)
  {
    cat('The ', rep.nb, '-th sample...\n')
    rep.dir <- paste0('rep_grandp-', rep.nb)
    data.dir <- file.path(in.dir, 'grandparents', rep.dir, paste0('skip_generation_national_raceth_total_pc_summary.csv'))
  ...
  data.all <- data.table::rbindlist( all.df.race, use.names = T, fill = T)
  set(data.all, NULL, 'sg', NULL)
  saveRDS(data.all, file.path(in.dir, 'grandparents', paste0('grandp_ACS_natioanl_race_pc_sample_all.RDS')))

  rm(all.df.race)

  # compute for the 95% bootstrap intervals at race & eth level
  pds.quantiles <- c(.025,.5,.975)
  ...
  cat('\nSaving the grandp at state level with quantiles...\n')
  saveRDS(data.all.state, file.path(in.dir, 'grandparents', paste0('grandp_ACS_national_race_pc_ci.RDS')))
}

get_95_ci_grandp_national_race_custodial_cg <- function(in.dir, nb.rep)
{
  all.df.race <- list()
  for (rep.nb in 1:nb.rep)
  {
    cat('The ', rep.nb, '-th sample...\n')
    rep.dir <- paste0('rep_grandp-', rep.nb)
    data.dir <- file.path(in.dir, 'grandparents', rep.dir, paste0('skip_generation_national_raceth_custodial_summary.csv'))
  ...
  data.all <- data.table::rbindlist( all.df.race, use.names = T, fill = T)
  set(data.all, NULL, 'sg', NULL)
  saveRDS(data.all, file.path(in.dir, 'grandparents', paste0('grandp_ACS_natioanl_race_custodial_sample_all.RDS')))
  ...
  cat('\nSaving the grandp at state level with quantiles...\n')
  saveRDS(data.all.state, file.path(in.dir, 'grandparents', paste0('grandp_ACS_national_race_custodial_ci.RDS')))
}

# process
get_95_ci_grandp_state_pry_cg( args$in.dir, args$nb.rep)
get_95_ci_grandp_national_race_pry_cg( args$in.dir, args$nb.rep)

get_95_ci_grandp_state_custodial_cg( args$in.dir, args$nb.rep)
get_95_ci_grandp_national_race_custodial_cg( args$in.dir, args$nb.rep)
stop()
