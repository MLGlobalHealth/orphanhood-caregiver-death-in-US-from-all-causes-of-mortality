...
generate_tableS3 <- function(do.inc.total, deaths.total, out.dir, if.rnk)
{
    ...
    tmp.cg[, prop := round(prop, 2)]
    tmp.cg[, prop := format(prop, digits = 2, nsmall = 2)]
...
generate_tableS3_wi <- function(do.inc.total, deaths.total, out.dir, if.rnk)
{
  cat('Processing for Supp Table 3 ...\n')
  tmp <- do.inc.total[year == 2021]
  tmp[, year.t := '2021']
  tmp[, cg.loss := output]
  tmp.cg <- tmp[, list(cg.loss = sum(cg.loss, na.rm = T)),
                by = c('cause.name', 'year.t', 'stat')]
  tmp.t <- tmp.cg[, list(cg.loss.t = sum(cg.loss, na.rm = T)), by = c('year.t', 'stat')]
  tmp.cg <- merge(tmp.cg, tmp.t, by = c('year.t', 'stat'))
  tmp.cg[, prop := cg.loss/cg.loss.t*100]
  tmp.cg[, prop := round(prop, 2)]
  tmp.cg[, prop := format(prop, digits = 2, nsmall = 2)]
  tmp.cg[, prop := paste0(prop, '%')]
