...
# Mothers ----
process_children_all_year = function(in.dir, prj.dir, cur.yr, country, is_child_mortality_needed, data_f)
{
  # construct a matrix, the rownames represent the
  children = matrix(rep(0, 100*18), nrow = 100)
  names(children) = paste0(seq(0:17), 'years')
  # TODO: update the following instruction!
  ...
  write.csv(children, paste0(file.path(in.dir, 'data/children', country), '_child_raw_f.csv'), row.names = F)
  ...
  if (is_child_mortality_needed){
    ...
    # child_m_matrix = read.csv(paste0(file.path(in.dir, 'data/children', 'raw', 'child_mortality_rate.csv')))
    child_and_m = as.matrix(children) * (1-as.matrix(child_m_matrix))
    child_and_m = as.data.frame(child_and_m)
    write.csv(child_and_m, paste0(file.path(in.dir, 'data/children', country), '_child_all_f.csv'), row.names = F)
  ...
  write.csv(plot_c_and_m, paste0(file.path(in.dir, 'data/children', country), '_child_all_list_f.csv'), row.names = F)
  setnames(plot_c_and_m, 'mother_age', 'parents_age')
  plott = read.csv(paste0(file.path(in.dir, 'data/children', country), '_child_all_list_m.csv'))
  setnames(plott, 'father_age', 'parents_age')
  plot_all = rbind(plot_c_and_m, plott)
  # contains the age of children and age of parents
  write.csv(plot_all, paste0(file.path(in.dir, 'data/children', country), '_child_all_list_both_sex.csv'), row.names = F)
  ...
  ddf$gender = 'female'
  ddf$age = 1:100
  write.csv(ddf, paste0(file.path(in.dir, 'data/children', country), '_children_f.csv'), row.names = F)

  ddf = read.csv(paste0(file.path(in.dir, 'data/children', country), '_children_f.csv'))
  ddf_2 = read.csv(paste0(file.path(in.dir, 'data/children', country),'_children_m.csv'))
  ...
  ggsave(filename = paste0(file.path(prj.dir, 'figures', 'children_'), tolower(country), ".pdf"), p, width = 6, height = 5)
  write.csv(ddf, paste0(file.path(in.dir, 'data/children', country),'_children.csv'), row.names = F)
}

# Fathers----
process_children_father_55_plus_year = function(in.dir, cur.yr, country, data_f){
  ##
  children = matrix(rep(0, 100*18), nrow = 100)
  names(children) = paste0(seq(0:17), 'years')
  ...
  write.csv(children, paste0(file.path(in.dir, 'data/children', country), '_child_raw_m.csv'), row.names = F)
  # plot_c = as.data.frame(as.numeric(as.character(unlist(children))))
