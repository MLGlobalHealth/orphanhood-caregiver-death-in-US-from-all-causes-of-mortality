...
process_adj_mort_data_cdc_state_race <- function(mort.data, cause.type, in.dir, prj.dir, imp.num)
{
  ...
  # tmp <- tmp[year > 2004, list(ratio.state = mean(ratio.state, na.rm = T)),
             # by = c('age', 'state', 'sex')]
  #
  tmp.state.adj <- merge(tmp[, list(year,age,state,sex,ratio.state)], d.drug, by = c('sex', 'state', 'age', 'year'), all = T)
  ...
  # update the race ethnicity composition based on the assumption where
  # the composition is stable across age years
  d.deaths.pre.t <- d.deaths.pre[, list(deaths.t = sum(deaths.nchs, na.rm = T)),
                                 by = c('sex', 'year', 'state')]
  d.deaths.pre.t <- merge(d.deaths.pre, d.deaths.pre.t, by = c('sex', 'year', 'state'), all = T)
  d.deaths.pre.t[, prop := deaths.nchs/deaths.t]

  # compare the NCHS race.eth prop with the adj CDC race.eth prop
  tmp.t <- tmp[, list(deaths.t = sum(deaths.cdc.adj, na.rm = T)),
                                 by = c('sex', 'year', 'state')]
  tmp.t <- merge(tmp, tmp.t, by = c('sex', 'year', 'state'), all = T)
  tmp.t[, prop:= deaths.cdc.adj/deaths.t]
  tmp.t <- tmp.t[year > 2004]
  cmp.prop <- rbind(d.deaths.pre.t, tmp.t, use.names = T, fill = T)
  col.race <- get_race_col()$col.race
  cmp.prop <- update_facet_sex(cmp.prop)
  p <- ggplot(cmp.prop, aes(x = year, y = prop, fill = race.eth)) +
  ...
  tmp.t[, rate :=  deaths.cdc.adj/pop*100]
  tmp.t <- tmp.t[year > 2004]
  cmp.prop <- rbind(d.deaths.pre.t, tmp.t, use.names = T, fill = T)
  ...
  # use the avg prop from 200-2004 to adj the race.eth composition
  ...
  d.drug.race.adj <- merge(tmp.state.adj[race.eth != 'Others'], cdc.rate, by = c('state', 'race.eth', 'sex'), all.x = T)
  d.drug.race.adj[, deaths.race.adj := deaths.adj/ratio]
  tmp <- d.drug.race.adj[race.eth != 'Others']
  # unique(d.drug$year)

  #
  # tmp <- tmp.state.adj[, list(deaths.cdc.adj.t = sum(deaths.adj, na.rm = T)),
  #               by = c('year', 'sex', 'state', 'age')]
  #
  # tmp.adj <- merge(tmp, cmp.prop, by = c('state', 'sex'), all.x = T, allow.cartesian = T)
  # tmp.adj[, deaths.race.adj := deaths.cdc.adj.t * prop ]
  # tmp <- merge(tmp.state.adj, tmp.adj, by = c('state', 'sex', 'year', 'age', 'race.eth'), all = T)
...
# fertility rates ----
process_usa_states_race_birth_comp <- function(prj.dir, in.dir, type.input, imp.num)
{
  imp.num
  # type.input <- 'state_race'
  if( !file.exists(
  ...
  if (1)
  {
    ...
    tmp <- tp[, list(births.raw.ratio = mean(births.raw.rate, na.rm = T),
                     births.imp.ratio = mean(births.imp.rate, na.rm = T)), by = c('sex', 'race.eth', 'state')]
  ...
  if (1)
  {
    ...
    tmp[, race.eth := gsub('-Hispanic ', '-Hispanic\n', race.eth)]
    tmp[, race.eth := gsub('or ', 'or\n', race.eth)]
    tmp <- tmp[race.eth != 'Others' & value > 0 & year %in% 1995:2004 & state %in% state.drug.show]
...
process_usa_states_race_birth_fertility_all_year_imputation <- function(prj.dir, in.dir, rep.nb, type.input)
{
  ...
  {
    process_usa_states_race_birth_comp(prj.dir, in.dir, type.input, imp.num = 1)
  }
  ...
  data.fill.fit <- merge(data.fill.fit, unique(fer[, list(state,race.eth)]), by = c('state', 'race.eth'), all.y = T)
  data.fill.fit[is.na(fertility_rate) & (year < 2005), table(state, year)]
...
analy_state_race <- function(prj.dir, mort.data)
{
  ...
  {
    tmp.sel <- as.data.table(readRDS(file.path(prj.dir, 'results', 'data_paper', paste0('state_race_', cause.type, '_topstates_mort_discp.rds'))))
    tmp <- rbind(tmp, tmp.sel)
  }
  ...
  tmp.sel <- as.data.table(readRDS(file.path(prj.dir, 'results', 'data_paper', paste0('state_race_topstates_births_discp.rds'))))
  ...
  tmp <- merge(tmp[, if.mort := T], tmp.sel, by = c('state', 'race.eth'), all = T)

  tp1 <- tmp[if.mort == T & !is.na(pass.num)]
  as.data.table(reshape2::dcast(tp1, state~race.eth, value.var = 'if.mort'))

  d.deaths <- d.deaths[year %in% (2004-17):2021]
  # filter the states by race.eth with reliable mortality data
  d.tmp <- d.deaths[, list(deaths = sum(deaths, na.rm = T)),
                    by = c('year', 'state', 'race.eth', 'cause.name')]
  d.tmp <- d.tmp[race.eth != 'Others' & year %in% 2004:2021, list(deaths.m = mean(deaths, na.rm = T)),
                 by = c('state', 'race.eth', 'cause.name')]
  d.tmp <- merge(d.tmp, tmp, by = c('state', 'race.eth'), all.x = T)
  d.tmp[, if.pick := deaths.m >= 10 & !is.na(pass.num) & if.mort == T]
  d.tmp <- d.tmp[if.pick == T]

  tmp <- reshape2::dcast(d.tmp, state~race.eth, value.var = 'if.pick')
  # write.csv(tmp, file.path(prj.dir, 'results', 'xx.csv'))

  d.deaths <- merge(d.deaths, d.tmp[, list(state,race.eth,cause.name, if.pick)], by = c('state', 'race.eth', 'cause.name'),
                    all.x = T)

  d.deaths <- d.deaths[if.pick == T]
