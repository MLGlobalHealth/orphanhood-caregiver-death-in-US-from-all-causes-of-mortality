...
# dot plot (Figure 3 EXTRA request: year 2019) ----

mort.data.simp <- as.data.table(readRDS(file.path(args$prj.dir, 'data', 'poisson_sampling_rnk',
                                                  'rep_id-0',  'rankable_cause_deaths_1983-2021.RDS')))
tmp <- mort.data.simp[year %in% 2019 , list(deaths.ttl = sum(deaths, na.rm = T)), by = c('sex', 'year', 'race.eth', 'cause.name')]
mean(tmp$deaths.ttl)
# 6568.83
tmp <- mort.data.simp[year %in% 2019 & cause.name != 'Others', list(deaths.ttl = sum(deaths, na.rm = T)), by = c('sex', 'year', 'race.eth', 'cause.name')]
mean(tmp$deaths.ttl)
# 5644.15  excluding 'Others'
