...
# in total the data are consistent, however, due to the suppression issue even at the national level
# there are small discrepancy here.
...
# tmp = readRDS(file.path(args$prj.dir, 'data', 'NCHS', 'death', 'output', 'Allcause_deaths_1983-2021.RDS'))
# tmp[year > 1999, sum(deaths)]
# [1] 57874666
d.deaths.nchs[year %in% 2000:2020 & age != '0-14' & grepl('Drug', cause.name), sum(deaths)]

# rankable_cause_deaths_1983-2021_raw_state_raceth.RDS
#. used the raw data
# run the script for the CDC WONDER national level data
if(!file.exists(
  file.path(args$prj.dir, 'data', 'CDC', 'ICD-10_113_Cause',
...
)
{
  ...
  if (!file.exists(file.path(args$prj.dir, 'data', 'CDC', 'ICD-10_113_Cause',
                             'US_state_no_race', 'national_rankable_causes.csv')))
  {
    ...
    write.csv(d.rankable, file.path(args$in.dir, 'CDC', 'ICD-10_113_Cause',
                                    'US_state_no_race', 'national_rankable_causes.csv'), row.names = F)
    #
    ...
    d.drug <- d.drug[, list(
      ...
      Gender,
      Year.Code)]
    # d.drug <- d.drug[Year.Code >= 2010]
    write.csv(d.drug, file.path(args$in.dir, 'CDC', 'ICD-10_113_Cause',
                                'US_state_no_race', 'national_drug-alcohol_causes.csv'), row.names = F)
    #
    ...
    write.csv(d.all, file.path(args$in.dir, 'CDC', 'ICD-10_113_Cause',
                               'US_state_no_race', 'national_alldeaths.csv'), row.names = F)
  }
  #
                             ...
                             'US_state_no_race', paste0(type.input, '_', 'leading-', args$sel.nb, 'causes_1999-2022.csv'))))
  {
    get_all_causes_deaths(main.path = file.path(args$in.dir, 'CDC', 'ICD-10_113_Cause',
                                                'US_state_no_race'), type.input, impute.supp = if.impute.supp, sel.nb = 'all')

  }
}

d.deaths.cdc <- as.data.table(read.csv(file.path(args$prj.dir, 'data', 'CDC', 'ICD-10_113_Cause',
                                                'US_state_no_race', 'national_leading-allcauses_1999-2022.csv')))
...
d.deaths.cdc[year %in% 2000:2020 & age != '0-14' & grepl('Drug', cause.name), sum(deaths)]
# due to suppression issue. if we ignore the suppressed cells, we will have 910502 counts;
# if we impute it, we will have 916422. the true counts are 912680
d.deaths.nchs[year %in% 2000:2020 & age != '0-14' & grepl('Drug', cause.name), sum(deaths)]

sum(d.deaths.nchs[year %in% 1999:2021 & age != '0-14']$deaths)
# 59448529
# sum(d.nchs[year > 1999]$deaths)

unique(d.deaths.cdc$age)
sum(d.deaths.cdc[year %in% 1999:2021 & age != '0-14']$deaths)
...
sum(tmp$nchs) #57098267
sum(tmp$cdc) #57098267
tmpp
...
p1
ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('edf_death_cdc_nchs_', type.input,'_pry_cause.png')), p1, w = 18, h = 13, dpi = 310, limitsize = FALSE)
ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('edf_death_cdc_nchs_', type.input,'_pry_cause.pdf')), p1, w = 18, h = 13, dpi = 310, limitsize = FALSE)
...
tmpp[year %in% 2000:2021, max(supp.rate)*100, by = 'sex']
# the same: 0 and 0
tmpp[supp.rate < 0]
