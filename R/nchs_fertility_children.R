...
# fertility rates
process_nchs_national_fertility <- function(in.dir, pop)
{
...
# alter approach 0728
process_nchs_national_race_fertility <- function(in.dir, pop)
{
  # load the NCHS births data
  cat('Loading Births data by NCHS... \n')
  if(!file.exists(
...
process_nchs_state_fertility <- function(in.dir, type.input, pop)
{
...
process_usa_states_national_birth_fertility_all_year = function(in.dir, cur.yr, type.input, rep.nb)#
{
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility_f_complete.csv'))
  ))
  {
    process_usa_states_national_birth_fertility_all_year_imputation(in.dir, type.input, rep.nb)
  }
...
process_usa_states_national_race_birth_fertility_all_year = function(in.dir, cur.yr, type.input, rep.nb)#
{
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility_f_complete.csv'))
  ))
  {
    process_usa_states_national_race_birth_fertility_all_year_imputation(in.dir, type.input, rep.nb)
  }
  cat("Saving fertility rates for current year ...\n")
  fert_f <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility_f_complete.csv'))
...
process_usa_states_national_race_stable_fertility_imput_all_year = function(in.dir, cur.yr, type.input, rep.nb)#
{
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0('national_race', '_', 'nchs_fertility_f_complete.csv'))
  ))
  {
    process_usa_states_national_race_birth_fertility_all_year_imputation(in.dir, 'national_race', rep.nb)
  }
  cat("Saving fertility rates for current year ...\n")
  fert_f <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0('national_race', '_', 'nchs_fertility_f_complete.csv'))
...
# 0731 run: fertility rates before 1990 by race have the same trend as the national fert rates
# won't use
process_usa_states_national_race_avg_fertility_imput_all_year = function(in.dir, cur.yr, type.input, rep=000)#
{
  # Load female complete fert data
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0('national_race', '_', 'nchs_fertility_f_complete.csv'))
  ))
  {
    process_usa_states_national_race_birth_fertility_all_year_imputation(in.dir, 'national_race', rep=000)
  }
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0('national', '_', 'nchs_fertility_f_complete.csv'))
  ))
  {
    process_usa_states_national_birth_fertility_all_year_imputation(in.dir, 'national', rep=000)
  }
  fert_f <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0('national_race', '_', 'nchs_fertility_f_complete.csv'))
  ))

  fert_f.n <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0('national', '_', 'nchs_fertility_f_complete.csv'))
  ))

  # load male complete fert data
  fert_m <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0('national_race', '_', 'nchs_fertility_m_complete.csv'))
  ))

  fert_m.n <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0('national', '_', 'nchs_fertility_m_complete.csv'))
  ))


  cat("Processing viz of fertility rates ratio ...\n")
  # show the ratio of fert_r to fert
  col.race <- c("#DF8F44FF", "#00A1D5FF", "#B24745FF", "#79AF97FF", "#D3D3D3" , '#4A6990FF')
  fert.comp <- merge(fert_f, fert_f.n, by = c('year', 'age', 'sex', 'state'), all = T)
  fert.comp[, ratio := fertility_rate.x/fertility_rate.y]

  fert.comp.m <- merge(fert_m, fert_m.n, by = c('year', 'age', 'sex', 'state'), all = T)
  fert.comp <- rbind(fert.comp, fert.comp.m, use.names = T, fill = T )
  fert.comp[, ratio := fertility_rate.x/fertility_rate.y]

  fert.comp.plt <- fert.comp[race.eth.x != 'Others']
  p <- ggplot(fert.comp.plt, aes(x = year, y = ratio, col = race.eth.x)) +
    geom_line() +
    geom_hline(yintercept = 1, col = 'grey50', linetype = 'dashed') +

    geom_vline(xintercept = 1990, col = 'grey50', linetype = 'dashed') +
    geom_vline(xintercept = 1980, col = 'grey50', linetype = 'dashed') +
    facet_grid(sex~age)  +
  scale_colour_manual(values = col.race, drop = T) +
    # scale_x_discrete(breaks = seq(min(pd$year), max(pd$year), 5)) +
    scale_y_continuous(limits = c(0, NA),
    ...
    theme_bw() +
    xlab('') +
    ylab('Fertility ratio of race/eth to national') +
    labs(col = 'Race & ethnicity') +
    guides(col = guide_legend(title.position="top", title.hjust = 0.5, nrow = 2)) +
    theme(legend.position = "bottom",
          axis.title = element_text(size = 16),
  ...
  ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('fert_race_impu_ratio.png')), p,  w = 12, h = 8, dpi = 310, limitsize = FALSE)
  ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('fert_race_impu_ratio.pdf')), p,  w = 12, h = 8, dpi = 310, limitsize = FALSE)

  # I suggested to use the 1990-1995 linear regression model to impute the previous fert rate ratio
  # but need to decide which year interval we would like to use for modelling...
  # for simplicity, just assume the stable ratio as in year 1990
  if (0)
  {
    # TODO: return to this part later
    fert.comp.plt <- fert.comp.plt[year %in% 1990:1999]
    dt <- fert.comp.plt[, list(intercept = summary(lm(ratio ~ year))$coefficients[1],
                               yearhat = summary(lm(ratio ~ year))$coefficients[2]),
                        by = c('age', 'sex', 'race.eth.x')]
    missing.fill <- list()
    i <- 0
    # race.cat[, dummy := 1]
    for (yr in seq( 1980,1989))
    {
      i <- i + 1
      tmp <- data.table(dt)
      tmp[, dummy := 1]
      # tmp <- merge(tmp, race.cat, by = 'dummy', allow.cartesian = T)
      tmp[, year:= yr]
      tmp[, ratio := intercept + yearhat * yr ]
    ...
    tmp[ratio < 0]
    tmp[ratio > 6]
    fert.comp.imp <- merge(tmp, fert.comp[year %in% 1980:1989], by = c('year', 'sex', 'age', 'race.eth.x'), all.y = T)
    fert.comp.imp[, fertility_rate := fertility_rate.y * ratio.x]
    # then assume the stable ratio, i.e. the same trends as the national fert
    tmp <- fert.comp.imp[year == 1980]
    set(tmp, NULL, 'year', NULL)
    tmp <- merge(tmp, fert.comp[year %in% 1966:1979], by = c('sex', 'age', 'race.eth.x'), all.y = T)
    tmp[, fertility_rate := fertility_rate.y.y * ratio.x]
    setnames(tmp, 'race.eth.x', 'race.eth')
    setnames(fert.comp.imp, 'race.eth.x', 'race.eth')
    setnames(fert.comp, 'race.eth.x', 'race.eth')

    setnames(fert.comp, 'fertility_rate.x', 'fertility_rate')
    fert.comp.all <- rbind(tmp[, list(year,age,sex,race.eth,fertility_rate)],
                           fert.comp.imp[, list(year,age,sex,race.eth,fertility_rate)],
                           fert.comp[year >= 1990, list(year,age,sex,race.eth,fertility_rate)])
    # add the national level fert rates
    fert.n <- rbind(fert_f.n, fert_m.n, use.names = T, fill = T)
    set(fert.n, NULL, c('state', 'race.eth'), NULL)
    setnames(fert.n, 'fertility_rate', 'fertility_rate.t')
    fert.comp.all.plt <- merge(fert.comp.all, fert.n, by = c('age', 'sex', 'year'), all.x = T)
    # viz
    fert.comp.all.plt <- fert.comp.all.plt[race.eth != 'Others']
    p <- ggplot(fert.comp.all.plt, aes(x = year, y = fertility_rate)) +
      geom_line(aes(y = fertility_rate, col = race.eth)) +
      geom_line(aes(y = fertility_rate.t), col = 'black') +
    ...
    # p
    ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('fert_race_impu_stable_linear_trends_ratio.png')), p,  w = 12, h = 8, dpi = 310, limitsize = FALSE)
    ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('fert_race_impu_stable_linear_trends_ratio.pdf')), p,  w = 12, h = 8, dpi = 310, limitsize = FALSE)


  }

  # here is the stable ratios
  if (1)
  {
    tmp <- fert.comp[year == 1990]
    set(tmp, NULL, 'year', NULL)
    tmp <- merge(tmp, fert.comp[year %in% 1966:1989], by = c('sex', 'age', 'race.eth.x'), all.y = T)
    tmp[, fertility_rate := fertility_rate.y.y * ratio.x]
    setnames(tmp, 'race.eth.x', 'race.eth')
    setnames(fert.comp, 'race.eth.x', 'race.eth')
    ...
    # p
    ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('fert_race_impu_stable_ratio.png')), p,  w = 12, h = 8, dpi = 310, limitsize = FALSE)
    ggsave(file.path(args$prj.dir, 'results', 'figs', paste0('fert_race_impu_stable_ratio.pdf')), p,  w = 12, h = 8, dpi = 310, limitsize = FALSE)

  }
  fert.comp.all[, state := 'National']
  fert_f <- fert.comp.all[ year %in% (cur.yr - 17):cur.yr & sex == 'Female']
  write.csv(fert_f,
            file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility_f_', cur.yr, '.csv'))
            , row.names = F)
  fert_m <- fert.comp.all[ year %in% (cur.yr - 17):cur.yr & sex == 'Male']
  write.csv(fert_m,
            file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility_m_', cur.yr, '.csv'))
            , row.names = F)
}

process_usa_states_birth_fertility_all_year = function(prj.dir, in.dir, cur.yr, type.input, rep.nb)#
{
  if (!file.exists(
    file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility_f_complete.csv'))
  ))
  ...
  if (type.input == 'state')
  {
    process_usa_states_birth_fertility_all_year_imputation(in.dir, type.input, rep.nb)
  }
  if (type.input == 'state_race')
  {
    process_usa_states_race_birth_fertility_all_year_imputation(prj.dir, in.dir, rep.nb, type.input)
  }
  }
  cat("Saving fertility rates for current year ...\n")
  fert_f <- as.data.table(read.csv(
...
process_usa_states_national_birth_fertility_all_year_imputation = function(in.dir, type.input, rep.nb)#
{
  # type.input = 'state'
  # type.input = 'national'
  # type.input = 'national_race'
  ...
  # ))
  {
    process_nchs_national_fertility(in.dir, pop)
  }
  # read fertility data
...
process_usa_states_national_race_birth_fertility_all_year_imputation = function(in.dir, type.input, rep.nb)#
{
  ...
  # ))
  {
    if (grepl('race', type.input))
    {
      process_nchs_national_race_fertility(in.dir, pop)
    }
  }
  # read fertility data
  fert <- as.data.table(read.csv(
...
process_usa_states_birth_fertility_all_year_imputation = function(in.dir, type.input, rep.nb)#
{
  # exclusively use the mortality data after year 1999 !!!
  # type.input = 'state'
  # type.input = 'national'
  ...
  # ))
  {
    process_nchs_state_fertility(in.dir, type.input, pop)
  }
  # read fertility data
  fert <- as.data.table(read.csv(
    file.path(in.dir, 'data','fertility', paste0(type.input, '_', 'nchs_fertility.csv'))
...
process_number_children_usa_state_national_all_year <- function(in.dir, prj.dir, cur.yr, type.input, rep.nb, folder.name)
{
  ...
  if (type.input == 'national')
  {
    ...
    process_usa_states_national_birth_fertility_all_year(in.dir, cur.yr, type.input, rep.nb)
  }

  if (type.input == 'national_race')
  {
    process_usa_states_national_race_birth_fertility_all_year(in.dir, cur.yr, type.input, rep.nb)
  }
  if (grepl('national_race_fert_stable', type.input))
  {
    process_usa_states_national_race_stable_fertility_imput_all_year(in.dir, cur.yr, type.input, rep.nb)
  }
...
process_number_children_usa_state_all_year <- function(in.dir, prj.dir, cur.yr, type.input, rep.nb, folder.name)
{
 ...
 {
   cat("Processing Fertility rates ...\n")
   process_usa_states_birth_fertility_all_year(prj.dir, in.dir, cur.yr, type.input, rep.nb)
 }
