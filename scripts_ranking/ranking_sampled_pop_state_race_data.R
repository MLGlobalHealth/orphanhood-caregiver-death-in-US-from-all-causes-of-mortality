...
# load the functions----
source(file.path(args$prj.dir, 'R', 'preprocess_pop_state_race_function.R'))
...
if (1)
{
  ...
  pop <- sample_pop_state_race_poisson_rnk(args$data.dir, rep.nb)
  ...
  for (i in unique(pop[idx > 0]$idx))
  {
    ...
    saveRDS(pop[idx == tmp.pick], file.path(args$out.dir, paste0('rep_id-', tmp.pick), 'state_race_cdc_population_5yr_old_all.rds'))
    saveRDS(pop.fert[idx == tmp.pick], file.path(args$out.dir, paste0('rep_id-', tmp.pick), 'state_race_cdc_population_5yr_all.rds'))
  }
}

# Save the data without poisson noise
if (!dir.exists(file.path(args$out.dir, paste0('rep_id-0'))))
{
  dir.create(file.path(args$out.dir, paste0('rep_id-0')))
}

saveRDS(pop[idx == 0], file.path(args$out.dir, paste0('rep_id-0'), 'state_race_cdc_population_5yr_old_all.rds'))
saveRDS(pop.fert[idx == 0], file.path(args$out.dir, paste0('rep_id-0'), 'state_race_cdc_population_5yr_all.rds'))
